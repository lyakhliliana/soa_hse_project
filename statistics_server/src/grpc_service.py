...
class StatisticsService(StatisticsServicer):
    ...
    async def GetTopPosts(self, request: GetTopPostsRequest, context) -> PostReply:
        ...
        async with async_session() as session:
            async with session.begin():
                ...
                query = select(database.post_id, database.author_login, cnt_func).group_by(
                    database.post_id).order_by(desc(cnt_func)).limit(5)
                top_posts = (await session.execute(query)).scalars().all()

        for post in top_posts:
            post_dict = post._asdict()
            yield PostReply(post_id=post_dict["post_id"], login=post_dict["author_login"], amount=post_dict["amount"])

    async def GetTopUsers(self, request: EmptyRequest, context) -> UserReply:
        ...
        async with async_session() as session:
            async with session.begin():
                ...
                top_users = (await session.execute(query)).scalars().all()
